...
js_code = js_code.replace('3.0, // High strength for the solid core', '1.2, // Lower strength for detail')
js_code = js_code.replace('0.2  // Low threshold', '0.6  // Higher threshold so it doesn\\'t blow out')

# 2. Spread out particles more to outer shells
js_code = js_code.replace('Math.pow(Math.random(), 5);', 'Math.pow(Math.random(), 2.0); // Less core bias to see outer neurons')
...
js_code = js_code.replace('gl_PointSize = (2.0 / -mvPosition.z) * (1.0 - aLayer * 0.8); // Core is brighter/bigger', 'gl_PointSize = (1.2 / -mvPosition.z) * (1.0 - aLayer * 0.5); // Core is brighter/bigger')
